...
function LoginContent() {
  ...
  async function onCredentials(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      // Bypass next-auth/react signIn helper which causes a full-page redirect
      // on failure in v5 beta, resetting React state and losing the error message.
  ...
  return (
    <div className="auth-container">
      <div className="w-full max-w-[400px]">
        ...
        <h1 className="text-2xl font-bold text-text-primary tracking-tight mb-1">Welcome back</h1>
        <p className="text-sm text-text-muted mb-6">Sign in to continue to your dashboard</p>

        {oauthError === "OAuthAccountNotLinked" && (
          <div className="mb-4 px-4 py-3 bg-negative-muted border border-negative/20 rounded-lg text-sm text-negative">
...
export default function LoginClientPage() {
  return (
    <Suspense fallback={<div className="auth-container"><div className="text-text-muted text-sm animate-pulse">Loading...</div></div>}>
      <LoginContent />
    </Suspense>
  );
}
